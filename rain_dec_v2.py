...
# Coordinates for your racetrack
...
# Human-readable mapping for common keys
param_map = {
    "temp": "Temperature (°C)",
    "hum": "Humidity (%)",
...
<head>
    ...
    <style>
        ...
        .graphs-container { display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; width: 100%; }
        .graph-item { display: flex; flex-direction: column; align-items: center; }
        .graph-item img { width: 100%; height: 300px; object-fit: contain; }
        .live-data { background-color: white; padding: 10px; border: 1px solid #ccc; border-radius: 8px; box-shadow: 0 2px 5px rgba(0,0,0,0.1); }
        ...
        <div id="Live" class="tabcontent">
            ...
            <div class="graphs-container">
                {% for metric, plot in plots.items() %}
                    ...
                    {% endif %}
                {% endfor %}
                <div class="graph-item live-data">
...
def index():
    ...
    for metric in metrics:
        ...
        if today_data or selected_day_data:
            fig, ax = plt.subplots(figsize=(6, 4))  # Increased size for larger plots
            if today_data:
                today_data.sort()
                x, y = zip(*today_data)
